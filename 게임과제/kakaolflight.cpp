...
// - 1~10단계
// - 8x8 논리를 16x16처럼 보이게(칸 사이 여백)
// - 내부 세로선 양끝 제외(보드 침범 X)
// - HUD: 우측 공간 부족 시 자동으로 "아래" 배치
// - "수용량 가드(ceil(rem/8))"로 큰 단계에서도 안정 배치
// - 성공: CONGRATULATION / 실패: TRY AGAIN / 종료: GOODBYE 배너
...
void splash_kakaol_flight(void){
    ...
    gotoxy(BASE_X+8, BASE_Y+10); printf("아무 키나 누르세요...");
